...
import { Upload, Plus, X, Briefcase, Clock, DollarSign, Github, Linkedin, Globe, ChevronRight, ChevronLeft, User, Camera } from 'lucide-react';
...
const Onboarding: React.FC = () => {
    ...
    // Projects
    const [projects, setProjects] = useState<{ title: string, link: string, description: string, image: string }[]>([
    ...
    const addProject = () => {
        setProjects([...projects, { title: '', link: '', description: '', image: '' }]);
    };
    ...
    const handleProjectImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, idx: number) => {
        const file = e.target.files?.[0];
        if (!file) return;
            ...
            <form onSubmit={handleSubmit} className="apple-blur border border-zinc-200 dark:border-white/10 rounded-[48px] shadow-2xl overflow-hidden min-h-[600px] flex flex-col transition-all duration-500">
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                                            <div className="flex items-center gap-2 text-white font-display text-xs font-bold uppercase tracking-widest">
                                                <Upload size={18} /> {loading ? 'Uploading...' : 'Change Banner'}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col md:flex-row gap-10 items-center md:items-start">
                                    ...
                                    <div className="flex flex-col items-center gap-4 shrink-0 relative -mt-16 md:-mt-20">
                                        ...
                                        <div
                                            ...
                                            className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-white dark:bg-zinc-900 border-[6px] border-white dark:border-[#0a0a0a] shadow-2xl flex items-center justify-center group cursor-pointer overflow-hidden relative"
                                        >
                                                ...
                                                >
                                                    {proj.image ? (
                                                        ...
                                                        <div className="flex flex-col items-center gap-2">
                                                            ...
                                                            <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">{loading ? 'Uploading...' : 'Upload Image'}</span>
                                                        </div>
                                                    )}
                                                    <input
                ...
                <div className="px-10 py-8 border-t border-zinc-200 dark:border-white/5 bg-zinc-50/50 dark:bg-black/20 flex justify-between items-center backdrop-blur-sm">
                    ...
                    {step < 3 ? (
                        ...
                        <Button type="submit" variant="primary" disabled={loading} className="gap-2 rounded-2xl px-12 py-4 text-xs font-bold font-display tracking-widest uppercase shadow-emerald-500/40">
                            {loading ? 'Launching...' : 'Launch Profile'} <Briefcase size={16} />
                        </Button>
                    )}
                </div>
            </form>
