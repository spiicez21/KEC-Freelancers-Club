...
const Portfolio: React.FC = () => {
    ...
    // Default fallback from mock profiles
    const defaultUser: any = mockProfiles[0];

    React.useEffect(() => {
        const fetchUserData = async () => {
            if (!id) {
                // If no ID in URL, show current user or fallback
            ...
            try {
                ...
                // If it's the current user, we already have it
    ...
    if (!user) {
        return <div className="text-center py-20 dark:text-white">User not found</div>;
    }
